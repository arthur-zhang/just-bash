...
/// Execute a block of statements (stub for now - will be implemented in statements.rs).
/// This is a minimal implementation to support function calls.
fn execute_block(ctx: &mut AwkContext, stmts: &[AwkStmt], program: &AwkProgram) {
    for stmt in stmts {
        if ctx.should_exit || ctx.has_return || ctx.loop_break || ctx.loop_continue {
            break;
        }
...
/// Execute a single statement (minimal implementation for function support).
fn execute_stmt(ctx: &mut AwkContext, stmt: &AwkStmt, program: &AwkProgram) {
    match stmt {
        AwkStmt::ExprStmt(expr) => {
            eval_expr(ctx, expr, program);
        }
        AwkStmt::Return(expr) => {
            ctx.return_value = expr.as_ref().map(|e| eval_expr(ctx, e, program));
            ctx.has_return = true;
        }
        AwkStmt::Block(stmts) => {
        ...
        AwkStmt::While { condition, body } => {
            ...
            while is_truthy(&eval_expr(ctx, condition, program)) {
                ...
                if ctx.has_return || ctx.should_exit {
                    break;
                }
            }
        }
        ...
        AwkStmt::Exit(expr) => {
            if let Some(e) = expr {
                ctx.exit_code = to_number(&eval_expr(ctx, e, program)) as i32;
            }
            ctx.should_exit = true;
        }
        _ => {
            // Other statements will be fully implemented in statements.rs
        }
    }
}
